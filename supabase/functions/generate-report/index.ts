...
function buildCategoryScoreLines(scores: AuditScores): string {
  const sortedCategories = [...scores.categories].sort(
    (a, b) => a.score - b.score,
  );
...
function buildFormContext(formState: FormState, isHS: boolean): string {
  ...
  if (isHS) {
    if (formState.step1.industry)
      formContext["industry"] = formState.step1.industry;
    if (formState.step1.employeeCount)
      formContext["employeeCount"] = formState.step1.employeeCount;
    if (formState.step1.annualRevenue)
      formContext["annualRevenue"] = formState.step1.annualRevenue;
    if (formState.step1.yearsInBusiness)
      formContext["yearsInBusiness"] = formState.step1.yearsInBusiness;
    if (formState.step1.serviceArea)
      formContext["serviceArea"] = formState.step1.serviceArea;
  } else {
    if (formState.step1.role) formContext["role"] = formState.step1.role;
    if (formState.step1.teamSize)
      formContext["teamSize"] = formState.step1.teamSize;
    if (formState.step1.transactionVolume)
      formContext["transactionVolume"] = formState.step1.transactionVolume;
    if (formState.step1.annualGCI)
      formContext["annualGCI"] = formState.step1.annualGCI;
    if (formState.step1.primaryMarket)
      formContext["primaryMarket"] = formState.step1.primaryMarket;
  }

  // Step 2: technology
  if (formState.step2.primaryCRM)
    formContext["primaryCRM"] = formState.step2.primaryCRM;
  if (formState.step2.crmSatisfaction)
    formContext["crmSatisfaction"] = `${formState.step2.crmSatisfaction}/5`;
  if (formState.step2.toolsUsed?.length)
    formContext["toolsUsed"] = formState.step2.toolsUsed;

  // Step 3: leads
  if (formState.step3.leadSources?.length)
    formContext["leadSources"] = formState.step3.leadSources;
  if (formState.step3.responseSpeed)
    formContext["leadResponseSpeed"] = formState.step3.responseSpeed;
  if (formState.step3.leadTracking)
    formContext["leadTracking"] = formState.step3.leadTracking;
  if (formState.step3.conversionRate)
    formContext["conversionRate"] = formState.step3.conversionRate;
  if (formState.step3.googleReviews)
    formContext["googleReviews"] = formState.step3.googleReviews;
  if (formState.step3.reviewAutomation)
    formContext["reviewAutomation"] = formState.step3.reviewAutomation;
  if (isHS && formState.step3.missedCallHandling)
    formContext["missedCallHandling"] = formState.step3.missedCallHandling;
  if (!isHS && formState.step3.touchesIn7Days)
    formContext["touchesIn7Days"] = formState.step3.touchesIn7Days;
  if (!isHS && formState.step3.leadDistribution)
    formContext["leadDistribution"] = formState.step3.leadDistribution;

  // Step 4: scheduling (HS) / lead management (RE)
  if (isHS) {
    if (formState.step4.schedulingMethod)
      formContext["schedulingMethod"] = formState.step4.schedulingMethod;
    if (formState.step4.dispatchMethod)
      formContext["dispatchMethod"] = formState.step4.dispatchMethod;
    if (formState.step4.routeOptimization)
      formContext["routeOptimization"] = formState.step4.routeOptimization;
    if (formState.step4.capacityPlanning)
      formContext["capacityPlanning"] = formState.step4.capacityPlanning;
  } else {
    if (formState.step4.followUpPlan)
      formContext["followUpPlan"] = formState.step4.followUpPlan;
    if (formState.step4.nurtureDuration)
      formContext["nurtureDuration"] = formState.step4.nurtureDuration;
    if (formState.step4.automatedDrip)
      formContext["automatedDrip"] = formState.step4.automatedDrip;
    if (formState.step4.leadTemperatureTracking)
      formContext["leadTemperatureTracking"] =
        formState.step4.leadTemperatureTracking;
    if (formState.step4.activityLogging)
      formContext["activityLogging"] = formState.step4.activityLogging;
  }

  // Step 5: communication
  if (formState.step5.internalComms)
    formContext["internalComms"] = formState.step5.internalComms;
  if (formState.step5.afterHoursComms)
    formContext["afterHoursComms"] = formState.step5.afterHoursComms;
  if (formState.step5.clientPortal)
    formContext["clientPortal"] = formState.step5.clientPortal;
  if (isHS) {
    if (formState.step5.appointmentReminders)
      formContext["appointmentReminders"] =
        formState.step5.appointmentReminders;
    if (formState.step5.onTheWayNotifications)
      formContext["onTheWayNotifications"] =
        formState.step5.onTheWayNotifications;
  } else {
    if (formState.step5.agentClientComms)
      formContext["agentClientComms"] = formState.step5.agentClientComms;
    if (formState.step5.transactionUpdates)
      formContext["transactionUpdates"] = formState.step5.transactionUpdates;
    if (formState.step5.pastClientEngagement)
      formContext["pastClientEngagement"] =
        formState.step5.pastClientEngagement;
  }

  // Step 6: follow-up & retention
  if (formState.step6.repeatBusinessPercent)
    formContext["repeatBusinessPercent"] =
      formState.step6.repeatBusinessPercent;
  if (isHS) {
    if (formState.step6.postJobFollowUp)
      formContext["postJobFollowUp"] = formState.step6.postJobFollowUp;
    if (formState.step6.serviceAgreements)
      formContext["serviceAgreements"] = formState.step6.serviceAgreements;
  } else {
    if (formState.step6.postCloseFollowUp)
      formContext["postCloseFollowUp"] = formState.step6.postCloseFollowUp;
    if (formState.step6.referralProcess)
      formContext["referralProcess"] = formState.step6.referralProcess;
    if (formState.step6.anniversaryTracking)
      formContext["anniversaryTracking"] = formState.step6.anniversaryTracking;
  }

  // Step 7: operations
  if (formState.step7.kpisTracked?.length)
    formContext["kpisTracked"] = formState.step7.kpisTracked;
  if (isHS) {
    if (formState.step7.performanceMeasurement)
      formContext["performanceMeasurement"] =
        formState.step7.performanceMeasurement;
    if (formState.step7.jobCosting)
      formContext["jobCosting"] = formState.step7.jobCosting;
  } else {
    if (formState.step7.agentPerformanceMeasurement)
      formContext["agentPerformanceMeasurement"] =
        formState.step7.agentPerformanceMeasurement;
    if (formState.step7.agentAccountability)
      formContext["agentAccountability"] = formState.step7.agentAccountability;
    if (formState.step7.transactionWorkflow)
      formContext["transactionWorkflow"] = formState.step7.transactionWorkflow;
  }

  // Step 8: financial
  if (formState.step8.paymentMethods?.length)
    formContext["paymentMethods"] = formState.step8.paymentMethods;
  if (formState.step8.financialReview)
    formContext["financialReview"] = formState.step8.financialReview;
  if (isHS) {
    if (formState.step8.estimateProcess)
      formContext["estimateProcess"] = formState.step8.estimateProcess;
    if (formState.step8.pricingModel)
      formContext["pricingModel"] = formState.step8.pricingModel;
    if (formState.step8.invoiceTiming)
      formContext["invoiceTiming"] = formState.step8.invoiceTiming;
  } else {
    if (formState.step8.expenseTracking)
      formContext["expenseTracking"] = formState.step8.expenseTracking;
    if (formState.step8.marketingBudget)
      formContext["marketingBudget"] = formState.step8.marketingBudget;
  }
...
function getItemCounts(overallScore: number): {
  gapCount: string;
  quickWinCount: string;
  stratRecCount: string;
} {
  if (overallScore < 40) {
...
function buildPrompt(params: {
  ...
  const { gapCount, quickWinCount, stratRecCount } =
    getItemCounts(overallScore);
...
const BG_SUB_NICHE_LABELS: Record<string, string> = {
  ...
  garage_doors:
    "\u0413\u0430\u0440\u0430\u0436\u043D\u0438 \u0432\u0440\u0430\u0442\u0438",
  pest_control:
    "\u0414\u0435\u0437\u0438\u043D\u0444\u0435\u043A\u0446\u0438\u044F \u0438 \u0434\u0435\u0437\u0438\u043D\u0441\u0435\u043A\u0446\u0438\u044F (DDD)",
  landscaping:
    "\u041E\u0437\u0435\u043B\u0435\u043D\u044F\u0432\u0430\u043D\u0435 \u0438 \u043F\u043E\u0434\u0434\u0440\u044A\u0436\u043A\u0430 \u043D\u0430 \u0433\u0440\u0430\u0434\u0438\u043D\u0438",
  cleaning:
  ...
  residential_sales:
    "\u0416\u0438\u043B\u0438\u0449\u043D\u0438 \u043F\u0440\u043E\u0434\u0430\u0436\u0431\u0438",
  commercial:
    "\u0422\u044A\u0440\u0433\u043E\u0432\u0441\u043A\u0438 \u0438\u043C\u043E\u0442\u0438 / \u041E\u0444\u0438\u0441\u0438",
  property_management:
    "\u0423\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u0438\u0435 \u043D\u0430 \u0438\u043C\u043E\u0442\u0438",
  new_construction:
    "\u041D\u043E\u0432\u043E \u0441\u0442\u0440\u043E\u0438\u0442\u0435\u043B\u0441\u0442\u0432\u043E",
  luxury_resort:
    "\u041B\u0443\u043A\u0441\u043E\u0437\u043D\u0438 / \u0412\u0430\u043A\u0430\u043D\u0446\u0438\u043E\u043D\u043D\u0438 \u0438\u043C\u043E\u0442\u0438",
};
...
function buildBulgarianPrompt(params: {
  ...
  const isHS = niche === "home_services";
  const nicheLabel = isHS
    ? "\u0431\u0438\u0437\u043D\u0435\u0441 \u0432 \u0441\u0444\u0435\u0440\u0430\u0442\u0430 \u043D\u0430 \u0434\u043E\u043C\u0430\u0448\u043D\u0438\u0442\u0435 \u0443\u0441\u043B\u0443\u0433\u0438"
    : "\u0435\u043A\u0438\u043F \u0437\u0430 \u043D\u0435\u0434\u0432\u0438\u0436\u0438\u043C\u0438 \u0438\u043C\u043E\u0442\u0438";
  const overallScore = scores.overall;
  const { gapCount, quickWinCount, stratRecCount } =
    getItemCounts(overallScore);
  ...
  try {
    ...
    if (!emailResult.success || !ipResult.success) {
      ...
      const hoursRemaining = Math.ceil(
        (failedResetMs - Date.now()) / (1000 * 60 * 60),
      );
    ...
    const techFrustrations = sanitizeText(
      formState?.step2?.techFrustrations,
      500,
    );
    const biggestChallenge = sanitizeText(
      formState?.step8?.biggestChallenge,
      500,
    );
    const niche = formState?.niche || "home_services";

    // Sub-niche: read from request body, resolve to human-readable label
    const subNicheKey: string | null =
      body.subNiche ?? body.formState?.subNiche ?? null;
    ...
    const subNicheLabelMap =
      language === "bg" ? BG_SUB_NICHE_LABELS : SUB_NICHE_LABELS;
    const subNicheLabel = subNicheKey
      ? (subNicheLabelMap[subNicheKey] ?? subNicheKey)
      : null;
    ...
    if (message.stop_reason === "max_tokens") {
      throw new Error(
        "AI response was truncated (max_tokens). Report too long for token limit.",
      );
    }

    // Extract and parse response
    let responseText = (message.content[0] as { type: string; text: string })
      .text;
    ...
    await supabaseAdmin.from("audit_reports").upsert(
      {
        audit_id: auditId,
        report: reportData,
      },
