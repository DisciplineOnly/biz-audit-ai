...
This requires the following keys in generating.json (to be added in Plan 01):
- `rateLimit.message`: "Вече изпратихте 3 одита днес. Опитайте отново {{timeHint}}."  (EN: "You've already submitted 3 audits today. Try again {{timeHint}}.")
- `rateLimit.timeHints.oneHour`: "след около 1 час" (EN: "in about 1 hour")
- `rateLimit.timeHints.hours`: "след около {{count}} часа" (EN: "in about {{count}} hours")
- `rateLimit.timeHints.tomorrow`: "утре" (EN: "tomorrow")

Also add these keys to the English generating.json (`public/locales/en/generating.json`) since the current English handling also needs to switch from server-provided message to client-side construction:

Add to en/generating.json under `rateLimit`:
```json
"message": "You've already submitted 3 audits today. Try again {{timeHint}}.",
"timeHints": {
...
- Rate-limit message is constructed client-side using i18n keys (works in both EN and BG)
- en/generating.json updated with rateLimit.message and rateLimit.timeHints keys
- Build passes
  </done>
</task>
