...
must_haves:
  truths:
    - "Sending 4 requests to generate-report with the same email within 24 hours causes the 4th to receive a 429 response (verified via curl)"
    - "Sending requests to generate-report from different email addresses succeeds without restriction (verified via curl)"
    - "Rate limit counters reset after 24 hours so a previously blocked email can submit again (verified by code inspection — fixedWindow TTL)"
    - "429 response includes friendly message with approximate time hint"
    - "Rate limit guard will enforce automatically on real user traffic once Phase 5 wires generate-report into Loading.tsx — no additional rate limit work needed"
  artifacts:
...
<objective>
Deploy the rate-limited generate-report edge function and verify all 3 Phase 4 success criteria via curl testing against the deployed edge function directly.
...
**Important context:** Loading.tsx does NOT currently call `generate-report` — that wiring is Phase 5. Verification in this plan uses direct curl calls to the edge function URL, which proves the rate limiting logic works end-to-end (Redis, counters, 429 response). Once Phase 5 wires `generate-report` into Loading.tsx, the guard enforces automatically on real user traffic with zero additional work.

Output: Deployed and verified rate-limited edge function (enforcement on user traffic pending Phase 5 wiring)
</objective>
...
- [ ] generate-report edge function deployed with rate limiting code (successful deploy = Deno type check pass)
...
Note: `npm run build` validates the Vite client bundle only, not edge function Deno code. Edge function TypeScript correctness is confirmed by successful deployment (Deno parses and loads the module at deploy time). There is no separate `deno check` step — deploy IS the type validation.
</verification>

<success_criteria>
The deployed generate-report edge function enforces SEC-01: email-based rate limiting at 3/24h and IP-based rate limiting at 10/24h, verified by direct curl tests showing 429 rejection on the 4th same-email request and successful request from a different email. Enforcement on real user traffic is pending Phase 5 wiring of generate-report into Loading.tsx — no additional rate limit code changes will be needed.

Note: Edge function TypeScript is validated by Deno runtime at deploy time, not by `npm run build` (which only checks the Vite client bundle). Successful MCP/CLI deploy confirms Deno can parse and execute the edge function code.
</success_criteria>
